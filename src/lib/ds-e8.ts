  ...
  experiments: [
    {
      ...
      content: {
        ...
        pretest: [
          { question: "What property must every node in a Binary Search Tree satisfy?", options: ["Left child is greater than parent and right child is less than parent", "All keys in the left subtree are less than the node's key and all keys in the right subtree are greater than the node's key", "All levels of the tree must be completely filled", "Every node must have exactly two children"], answerIndex: 1, hint: "Think about why the correct answer involves: All keys in the left subtree are less than the node's key and all keys in the right subtree are grea..." },
          { question: "When a new key is inserted into a BST it is always placed at:", options: ["The root of the tree", "A leaf node position", "The leftmost internal node", "Depends on the key value relative to the root only"], answerIndex: 1, hint: "Think about why the correct answer involves: A leaf node position" },
          { question: "Which traversal of a BST produces keys in ascending sorted order?", options: ["Preorder traversal", "Postorder traversal", "Inorder traversal (Left → Root → Right)", "Level order traversal"], answerIndex: 2, hint: "Think about why the correct answer involves: Inorder traversal (Left → Root → Right)" },
          { question: "For a BST with root 50 and keys {30, 70} already inserted, where is key 40 placed when inserted?", options: ["Left child of root 50", "Right child of node 30", "Left child of node 70", "Left child of node 30"], answerIndex: 1, hint: "Think about why the correct answer involves: Right child of node 30" },
          { question: "What is the worst-case time complexity for inserting n keys one by one into a BST?", options: ["O(n log n)", "O(log n) for each insertion always", "O(n) per insertion leading to O(n²) total when keys are inserted in sorted order", "O(1) per insertion"], answerIndex: 2, hint: "Think about why the correct answer involves: O(n) per insertion leading to O(n²) total when keys are inserted in sorted order" }
        ],
        procedure: [
        ...
        posttest: [
          { question: "After inserting keys 50, 30, 70, 20, 40, 60, 80 into a BST what is the Postorder traversal output?", options: ["20 30 40 50 60 70 80", "50 30 20 40 70 60 80", "20 40 30 60 80 70 50", "80 60 70 40 20 30 50"], answerIndex: 2, hint: "Think about why the correct answer involves: 20 40 30 60 80 70 50" },
          { question: "If keys are inserted in the order 1, 2, 3, 4, 5 into a BST what is the height of the resulting tree?", options: ["3 (balanced height for 5 nodes)", "5 — the tree degenerates into a right-skewed linked list with height equal to n", "2", "log₂(5) ≈ 2"], answerIndex: 1, hint: "Think about why the correct answer involves: 5 — the tree degenerates into a right-skewed linked list with height equal to n" },
          { question: "Which traversal would you use to safely delete all nodes in a BST without causing memory errors?", options: ["Preorder — delete root before children", "Inorder — delete in sorted order", "Postorder — children are deleted before their parent so no node is freed while its subtree still exists", "Any traversal works equally well for deletion"], answerIndex: 2, hint: "Think about why the correct answer involves: Postorder — children are deleted before their parent so no node is freed while its subtree still exi..." },
          { question: "A BST is built by inserting keys 15, 10, 20, 8, 12. What is the Preorder traversal?", options: ["8 10 12 15 20", "15 10 8 12 20", "8 12 10 20 15", "20 15 10 12 8"], answerIndex: 1, hint: "Think about why the correct answer involves: 15 10 8 12 20" },
          { question: "How does the linked list representation of a BST differ from the array representation in terms of memory usage?", options: ["Linked representation always uses less memory than array representation", "Array representation is always more memory efficient", "Linked representation uses dynamic allocation — memory is proportional to actual nodes inserted with no wasted space, but each node has pointer overhead; array representation pre-allocates for maximum size wasting space for sparse trees but has no pointer overhead", "Both representations use identical memory"], answerIndex: 2, hint: "Think about why the correct answer involves: Linked representation uses dynamic allocation — memory is proportional to actual nodes inserted with..." }
        ],
        references: [
