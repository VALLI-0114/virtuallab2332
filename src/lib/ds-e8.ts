  ...
  experiments: [
    {
      ...
      content: {
        ...
        pretest: [
          { question: "What property must every node in a Binary Search Tree satisfy?", options: ["Left child is greater than parent and right child is less than parent", "All keys in the left subtree are less than the node's key and all keys in the right subtree are greater than the node's key", "All levels of the tree must be completely filled", "Every node must have exactly two children"], answerIndex: 1 },
          { question: "When a new key is inserted into a BST it is always placed at:", options: ["The root of the tree", "A leaf node position", "The leftmost internal node", "Depends on the key value relative to the root only"], answerIndex: 1 },
          { question: "Which traversal of a BST produces keys in ascending sorted order?", options: ["Preorder traversal", "Postorder traversal", "Inorder traversal (Left → Root → Right)", "Level order traversal"], answerIndex: 2 },
          { question: "For a BST with root 50 and keys {30, 70} already inserted, where is key 40 placed when inserted?", options: ["Left child of root 50", "Right child of node 30", "Left child of node 70", "Left child of node 30"], answerIndex: 1 },
          { question: "What is the worst-case time complexity for inserting n keys one by one into a BST?", options: ["O(n log n)", "O(log n) for each insertion always", "O(n) per insertion leading to O(n²) total when keys are inserted in sorted order", "O(1) per insertion"], answerIndex: 2 }
        ],
        procedure: [
