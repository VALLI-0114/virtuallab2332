  ...
  experiments: [
    {
      ...
      content: {
        ...
        pretest: [
          { question: "How many pointer fields does each node in a doubly linked list contain?", options: ["1 — only next pointer", "2 — prev and next pointers", "3 — prev, next, and head pointers", "Depends on the number of elements"], answerIndex: 1, hint: "Think about why the correct answer involves: 2 — prev and next pointers" },
          { question: "What is the prev pointer value of the first node in a doubly linked list?", options: ["The address of the last node", "The address of head", "NULL", "The address of the second node"], answerIndex: 2, hint: "Think about why the correct answer involves: NULL" },
          { question: "What is the key advantage of doubly linked list over singly linked list for deletion?", options: ["Doubly linked list uses less memory", "A node can be deleted in O(1) given only its pointer because its predecessor is accessible via node→prev without traversal from head", "Doubly linked list allows faster searching", "Doubly linked list can store more data per node"], answerIndex: 1, hint: "Think about why the correct answer involves: A node can be deleted in O(1) given only its pointer because its predecessor is accessible via node→..." },
          { question: "After inserting a new node between nodes A and B in a doubly linked list how many pointer assignments are needed?", options: ["1", "2", "3", "4"], answerIndex: 3, hint: "Think about why the correct answer involves: 4" },
          { question: "What is the time complexity of backward traversal of a doubly linked list of n nodes?", options: ["O(1)", "O(log n)", "O(n)", "O(n²)"], answerIndex: 2, hint: "Think about why the correct answer involves: O(n)" }
        ],
        procedure: [
        ...
        posttest: [
          { question: "A doubly linked list has nodes A ↔ B ↔ C ↔ D. After deleting node B what are the correct pointer updates?", options: ["A→next = C only", "A→next = C and C→prev = A", "A→next = C and B→prev = NULL", "C→prev = NULL and A→next = B→next"], answerIndex: 1, hint: "Think about why the correct answer involves: A→next = C and C→prev = A" },
          { question: "How many total pointer fields are used in a doubly linked list of n nodes?", options: ["n", "2n", "n+1", "n²"], answerIndex: 1, hint: "Think about why the correct answer involves: 2n" },
          { question: "What is printed by backward traversal of the list 1 ↔ 2 ↔ 3 ↔ 4 ↔ 5?", options: ["1 2 3 4 5", "5 4 3 2 1", "5 3 1", "4 3 2 1"], answerIndex: 1, hint: "Think about why the correct answer involves: 5 4 3 2 1" },
          { question: "When inserting a node newNode after node cur in a doubly linked list which pointer assignment must come first to avoid losing the reference to cur→next?", options: ["cur→next = newNode", "newNode→prev = cur", "newNode→next = cur→next", "cur→next→prev = newNode"], answerIndex: 2, hint: "Think about why the correct answer involves: newNode→next = cur→next" },
          { question: "In which real-world application is the doubly linked list's backward traversal capability most directly useful?", options: ["Implementing a stack for expression evaluation", "Browser forward and backward navigation history where both directions of traversal are needed", "Implementing a hash table with chaining", "Implementing a circular buffer for streaming data"], answerIndex: 1, hint: "Think about why the correct answer involves: Browser forward and backward navigation history where both directions of traversal are needed" }
        ],
        references: [
    ...
    },
    {
      ...
      content: {
        ...
        pretest: [
          { question: "In a circular singly linked list what does the next pointer of the last node point to?", options: ["NULL", "The last node itself", "The first node (head)", "The tail pointer variable"], answerIndex: 2, hint: "Think about why the correct answer involves: The first node (head)" },
          { question: "Why is maintaining a tail pointer more efficient than a head pointer for a circular linked list?", options: ["Tail pointer uses less memory", "With a tail pointer both the last node (tail) and first node (tail→next) are accessible in O(1) enabling O(1) operations at both ends", "Head pointer cannot be used with circular linked lists", "Tail pointer automatically maintains the circular invariant"], answerIndex: 1, hint: "Think about why the correct answer involves: With a tail pointer both the last node (tail) and first node (tail→next) are accessible in O(1) enab..." },
          { question: "What traversal loop pattern is most natural for a circular linked list?", options: ["for loop with index i from 0 to n-1", "while(cur != NULL) — standard linear traversal", "do-while(cur != head) — visit current node then advance, stop when returning to start", "while(cur→next != cur) — stop when only one node remains"], answerIndex: 2, hint: "Think about why the correct answer involves: do-while(cur != head) — visit current node then advance, stop when returning to start" },
          { question: "When inserting a new node at the beginning of a circular linked list using a tail pointer, which pointer must be updated to maintain the circular invariant?", options: ["tail→next only", "newNode→next and tail→next", "newNode→next only", "tail and newNode→prev"], answerIndex: 1, hint: "Think about why the correct answer involves: newNode→next and tail→next" },
          { question: "Which real-world system most naturally uses a circular linked list as its underlying data model?", options: ["A stack for recursive function calls", "A binary search tree for sorted data", "A round-robin CPU scheduler where each process gets a time slice and control cycles back to the first process after the last", "A hash table for fast key-value lookup"], answerIndex: 2, hint: "Think about why the correct answer involves: A round-robin CPU scheduler where each process gets a time slice and control cycles back to the firs..." }
        ],
        procedure: [
        ...
        posttest: [
          { question: "A circular linked list has nodes 1 → 2 → 3 → 4 and last→next = node(1). After deleting from the end what does the new last node's next pointer contain?", options: ["NULL", "Address of node(4) which was just freed", "Address of node(1) — the circular link is maintained to point back to head", "Address of node(3)"], answerIndex: 2, hint: "Think about why the correct answer involves: Address of node(1) — the circular link is maintained to point back to head" },
          { question: "What is the danger of not properly bounding the traversal loop in a circular linked list?", options: ["The program will crash immediately on the first iteration", "The traversal will stop at the last node which has NULL next", "The traversal will loop infinitely visiting nodes forever because there is no NULL terminator", "The traversal will skip alternate nodes"], answerIndex: 2, hint: "Think about why the correct answer involves: The traversal will loop infinitely visiting nodes forever because there is no NULL terminator" },
          { question: "A circular linked list with tail pointer has nodes [10, 20, 30, 40]. What does tail→next→next→data equal?", options: ["10", "20", "30", "40"], answerIndex: 1, hint: "Think about why the correct answer involves: 20" },
          { question: "After deleting the only remaining node from a circular linked list what value should tail be set to?", options: ["tail→next (pointing to itself)", "head", "NULL", "0"], answerIndex: 2, hint: "Think about why the correct answer involves: NULL" },
          { question: "A round-robin scheduler has 4 processes P1 P2 P3 P4 in a circular linked list. After P4 executes which process executes next and how does the circular linked list model this?", options: ["The scheduler stops — P4 is the last node with next=NULL", "P1 executes next — because P4→next points back to P1 in the circular list naturally modeling the cycling behavior without any special end-of-list handling", "P3 executes next — the scheduler reverses direction", "A new process is created — circular lists automatically grow when the tail is reached"], answerIndex: 1, hint: "Think about why the correct answer involves: P1 executes next — because P4→next points back to P1 in the circular list naturally modeling the cyc..." }
        ],
        references: [
          "Weiss M.A., Data Structures and Algorithm Analysis in C, 2nd Edition, Pearson",
          "Horowitz E., Sahni S., and Anderson-Freed S., Fundamentals of Data Structures in C, Silicon Press, 2008",
