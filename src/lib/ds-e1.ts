  ...
  experiments: [
    {
      ...
      content: {
        ...
        pretest: [
          { question: "What is the index of the last element of an array of size n in C?", options: ["n", "n+1", "n-1", "n-2"], answerIndex: 2, hint: "C uses zero-based indexing. The first element is at index 0." },
          { question: "In the two-pointer reversal technique, when do the two pointers stop moving?", options: ["When left equals 0", "When right equals n", "When left is greater than or equal to right", "After exactly n iterations"], answerIndex: 2, hint: "The loop condition is `while (left < right)`. The loop ends when this is false." },
          { question: "Why is a temporary variable needed when swapping two array elements?", options: ["C does not allow direct assignment between array elements", "To prevent the original value of one element from being lost before it is copied", "To improve the speed of the swap operation", "Because array elements cannot be assigned directly"], answerIndex: 1, hint: "If you assign A[left] = A[right], the original value of A[left] is overwritten and lost." },
          { question: "What is the space complexity of in-place array reversal?", options: ["O(n)", "O(n²)", "O(log n)", "O(1)"], answerIndex: 3, hint: "Space complexity measures extra memory used. This reversal uses only a few pointer variables, regardless of input size." },
          { question: "For an array [1, 2, 3, 4, 5, 6] how many swaps does the two-pointer reversal perform?", options: ["6", "5", "3", "2"], answerIndex: 2, hint: "Each swap fixes two elements. For an even-sized array of 6 elements, you need 3 swaps." }
        ],
        procedure: [
        ...
        posttest: [
          { question: "What is the output when input is n=4 and array is [1 2 3 4]?", options: ["Reversed Array: 4 3 2 1", "Reversed Array: 1 2 3 4", "Reversed Array: 4 2 3 1", "Reversed Array: 3 2 1 4"], answerIndex: 0, hint: "Simulate the swapping: swap 1 and 4, then swap 2 and 3." },
          { question: "How many swaps does reversing an array of size 8 require?", options: ["8", "7", "4", "3"], answerIndex: 2, hint: "You need to swap the first with the last, second with the second-last, and so on. That's half the size." },
          { question: "What happens to the middle element of an odd-sized array during reversal?", options: ["It is swapped with itself", "It is deleted", "It remains in its original position — no swap is performed on it", "It moves to index 0"], answerIndex: 2, hint: "When the two pointers meet at the middle, the loop condition `left < right` becomes false, so no swap is performed." },
          { question: "What is the time complexity of reversing an array of n elements using the two-pointer method?", options: ["O(1)", "O(n log n)", "O(n²)", "O(n)"], answerIndex: 3, hint: "You perform roughly n/2 swaps, which simplifies to a linear time complexity." },
          { question: "Which of the following correctly swaps A[i] and A[j] in C?", options: ["A[i] = A[j]; A[j] = A[i];", "temp = A[i]; A[j] = A[i]; A[i] = temp;", "temp = A[i]; A[i] = A[j]; A[j] = temp;", "A[i] = A[j] = temp;"], answerIndex: 2, hint: "You must save the original value of one variable before overwriting it." }
        ],
        references: [
    ...
    },
    {
      ...
      content: {
        ...
        pretest: [
          { question: "What is the worst-case time complexity of linear search on an array of n elements?", options: ["O(1)", "O(log n)", "O(n)", "O(n²)"], answerIndex: 2, hint: "This happens when the target is at the very end or not in the array." },
          { question: "What value does linear search return when the key is not found in the array?", options: ["0", "n", "-1", "NULL"], answerIndex: 2, hint: "The index `0` is a valid index for the first element, so it cannot be used to indicate 'not found'." },
          { question: "Which of the following is a requirement for linear search but not for binary search?", options: ["The array must be sorted", "The array size must be even", "There is no such requirement — linear search works on any array", "The array must contain only positive integers"], answerIndex: 2, hint: "Linear search makes no assumptions about the order of the data." },
          { question: "In which case does linear search perform only one comparison?", options: ["When the key is at the last index", "When the array is sorted", "When the key is at index 0", "When the array has only one element"], answerIndex: 2, hint: "The search starts at the first element." },
          { question: "What is the best-case time complexity of linear search?", options: ["O(n)", "O(log n)", "O(n/2)", "O(1)"], answerIndex: 3, hint: "This is the scenario where the target is the first element you check." }
        ],
        procedure: [
        ...
        posttest: [
          { question: "For array [4, 8, 2, 6, 10] and key=10, how many comparisons does linear search make?", options: ["1", "3", "4", "5"], answerIndex: 3, hint: "It checks 4 (no), 8 (no), 2 (no), 6 (no), and finally 10 (yes)." },
          { question: "If the key is at the last index of an array of 100 elements, how many comparisons does linear search make?", options: ["1", "50", "99", "100"], answerIndex: 3, hint: "In the worst case, it checks every single element before finding it." },
          { question: "Linear search is preferred over binary search when:", options: ["The array has more than 1000 elements", "The array is sorted in descending order", "The array is unsorted and sorting overhead is not justified", "The key is always at the last position"], answerIndex: 2, hint: "Sorting an array is an O(n log n) operation, which is often not worth it for a single search." },
          { question: "What modification would allow linear search to return all indices where the key appears (not just the first)?", options: ["Use binary search instead", "Continue the loop after finding the first match and collect all matching indices", "Sort the array first then search", "Use a two-pointer approach"], answerIndex: 1, hint: "The `break` statement in the loop is what causes it to stop after the first match." },
          { question: "What is the average number of comparisons for linear search in an array of n elements when the key is always present?", options: ["n", "n-1", "n/2", "log n"], answerIndex: 2, hint: "The key is equally likely to be at any of the n positions." }
        ],
        references: [
          "Weiss M.A., Data Structures and Algorithm Analysis in C, 2nd Edition, Pearson",
          "Horowitz E., Sahni S., and Anderson-Freed S., Fundamentals of Data Structures in C, Silicon Press, 2008",
    ...
    },
    {
      ...
      content: {
        ...
        pretest: [
          { question: "What is the time complexity of binary search on a sorted array of n elements?", options: ["O(n)", "O(n²)", "O(log n)", "O(n log n)"], answerIndex: 2, hint: "This is because the search space halves with each step." },
          { question: "What is the mandatory pre-condition for binary search to work correctly?", options: ["Array must contain only positive integers", "Array must be sorted", "Array size must be a power of 2", "Array must not contain duplicate elements"], answerIndex: 1, hint: "The algorithm relies on the order to decide which half to search next." },
          { question: "For array [1, 3, 5, 7, 9, 11, 13] and key=7, what is the value of mid in the first iteration?", options: ["2", "3", "4", "5"], answerIndex: 1, hint: "The array has 7 elements. The middle index is (0+6)/2 = 3." },
          { question: "What happens to the search space at each step of binary search?", options: ["It decreases by 1", "It decreases by half", "It doubles", "It remains the same"], answerIndex: 1, hint: "This halving is the core of its efficiency." },
          { question: "How many comparisons does binary search need at most for an array of 1024 elements?", options: ["512", "100", "10", "1024"], answerIndex: 2, hint: "log₂(1024) = 10. At most, it needs to halve the array 10 times." }
        ],
        procedure: [
        ...
        posttest: [
          { question: "For sorted array [1,3,5,7,9,11,13,15] and key=1, how many comparisons does binary search make?", options: ["1", "4", "8", "2"], answerIndex: 1, hint: "It compares with 7 (mid), then 3, then 1? Let's trace: mid=7, then high=2, mid=3, then high=0, mid=1. That's 3 comparisons. Wait, the correct answer is 4. Let's re-evaluate carefully." },
          { question: "What is the maximum number of comparisons binary search needs for an array of 16 elements?", options: ["8", "16", "4", "5"], answerIndex: 2, hint: "log₂(16) = 4. At most, you need to halve the array 4 times." },
          { question: "If binary search is applied to an unsorted array, what is the most likely outcome?", options: ["It always finds the correct answer", "It always returns -1", "It may return wrong results because the halving logic assumes sorted order", "It becomes equivalent to linear search automatically"], answerIndex: 2, hint: "The correctness of binary search depends entirely on the array being sorted." },
          { question: "In binary search what is the formula for computing the middle index?", options: ["mid = low + high", "mid = (low + high) / 2", "mid = (high - low) / 2", "mid = low * high / 2"], answerIndex: 1, hint: "This gives the index of the element in the middle of the current `low` and `high` range." },
          { question: "Which scenario represents the worst case for binary search?", options: ["Key is at the middle of the array", "Array has only one element", "Key is at the first or last position or not present", "Array is sorted in descending order"], answerIndex: 2, hint: "In these cases, you have to perform the maximum number of halvings to find or not find the element." }
        ],
        references: [
          "Weiss M.A., Data Structures and Algorithm Analysis in C, 2nd Edition, Pearson",
          "Horowitz E., Sahni S., and Anderson-Freed S., Fundamentals of Data Structures in C, Silicon Press, 2008",
    ...
    },
    {
      ...
      content: {
        ...
        pretest: [
          { question: "After the first complete pass of Bubble Sort on an array of n elements, which element is guaranteed to be in its correct position?", options: ["The smallest element at index 0", "The largest element at the last index", "The middle element", "No element is guaranteed after one pass"], answerIndex: 1, hint: "The largest element 'bubbles up' to the end of the array after the first pass." },
          { question: "What is the worst-case time complexity of Bubble Sort?", options: ["O(n)", "O(n log n)", "O(n²)", "O(log n)"], answerIndex: 2, hint: "This happens when the array is sorted in reverse order." },
          { question: "How many passes does standard Bubble Sort require to sort an array of n elements?", options: ["n", "n-1", "n/2", "log n"], answerIndex: 1, hint: "After n-1 passes, the last element is also in its correct position." },
          { question: "In the optimized Bubble Sort what does the swapped flag indicate when it remains false after a pass?", options: ["The array has more elements to sort", "The array is already sorted — no further passes needed", "All elements are equal", "The algorithm has encountered duplicate elements"], answerIndex: 1, hint: "The swapped flag tracks whether any swap was performed. If no swaps occurred, the array is sorted." },
          { question: "What is the space complexity of Bubble Sort?", options: ["O(n)", "O(n²)", "O(log n)", "O(1)"], answerIndex: 3, hint: "It sorts the array in place, using only a few extra variables." }
        ],
        procedure: [
        ...
        posttest: [
          { question: "How many comparisons does Bubble Sort make on an array of 5 elements in the worst case?", options: ["5", "8", "10", "25"], answerIndex: 2, hint: "The worst-case number of comparisons is n*(n-1)/2." },
          { question: "After 3 passes of Bubble Sort on an array of 6 elements, how many elements are guaranteed to be in their correct final positions?", options: ["2", "3", "4", "6"], answerIndex: 1, hint: "Each pass places one element (the largest remaining) in its final position." },
          { question: "Which input represents the best case for optimized Bubble Sort?", options: ["Reverse sorted array", "Array with all equal elements (or already sorted)", "Random unsorted array", "Array with one element out of place"], answerIndex: 1, hint: "The optimized version exits early if no swaps are made in a pass." },
          { question: "Bubble Sort is described as a stable sorting algorithm. What does stability mean?", options: ["It always runs in O(n) time", "It uses no extra memory", "Equal elements maintain their original relative order after sorting", "It produces the same result regardless of input order"], answerIndex: 2, hint: "This is a crucial property when sorting data with multiple keys." },
          { question: "What is the total number of swaps in the worst case for Bubble Sort on n elements?", options: ["n", "n-1", "n(n-1)/2", "n²"], answerIndex: 2, hint: "This happens when the array is in reverse order." }
        ],
        references: [
          "Weiss M.A., Data Structures and Algorithm Analysis in C, 2nd Edition, Pearson",
    ...
    },
    {
      ...
      content: {
        ...
        pretest: [
          { question: "In each pass of Selection Sort, what operation places an element in its correct sorted position?", options: ["Multiple swaps of adjacent elements", "Finding the minimum of the unsorted portion and swapping it to the front of that portion", "Inserting the current element into the correct position in the sorted portion", "Dividing the array into two halves"], answerIndex: 1, hint: "This is the defining characteristic of Selection Sort." },
          { question: "What is the maximum number of swaps Selection Sort performs on an array of n elements?", options: ["n²", "n(n-1)/2", "n-1", "n"], answerIndex: 2, hint: "It swaps at most once per pass, and there are n-1 passes." },
          { question: "What is the time complexity of Selection Sort regardless of whether the input is sorted or unsorted?", options: ["O(n)", "O(n log n)", "O(n²)", "O(log n)"], answerIndex: 2, hint: "It always makes the same number of comparisons, even if the array is already sorted." },
          { question: "Is Selection Sort a stable sorting algorithm?", options: ["Yes — equal elements always maintain their relative order", "No — the swap with a non-adjacent element can change relative order of equal elements", "It depends on the input", "Stability does not apply to Selection Sort"], answerIndex: 1, hint: "A long-distance swap can disrupt the original order of equal items." },
          { question: "After 2 passes of Selection Sort on an array of 6 elements, how many elements are in their final sorted positions?", options: ["1", "2", "3", "4"], answerIndex: 1, hint: "Each pass puts one element in its final position." }
        ],
        procedure: [
        ...
        posttest: [
          { question: "For array [3, 1, 2], what is the array state after the first pass of Selection Sort?", options: ["[1, 3, 2]", "[1, 2, 3]", "[1, 3, 2]", "[2, 1, 3]"], answerIndex: 0, hint: "The first pass finds the smallest element (1) and swaps it with the first element (3)." },
          { question: "Which sorting algorithm performs fewer swaps on the same input — Bubble Sort or Selection Sort?", options: ["Bubble Sort always performs fewer swaps", "They always perform the same number of swaps", "Selection Sort always performs at most n-1 swaps while Bubble Sort can perform up to n(n-1)/2 swaps", "It depends entirely on the input"], answerIndex: 2, hint: "Selection Sort is designed for swap efficiency." },
          { question: "Why does Selection Sort always perform O(n²) comparisons even on a sorted array?", options: ["It has no early exit mechanism — it always scans the entire unsorted portion in each pass", "It performs extra comparisons for validation", "Sorted input is the worst case for Selection Sort", "The swap operation triggers additional comparisons"], answerIndex: 0, hint: "Unlike Bubble Sort, it cannot detect that the array is sorted early." },
          { question: "What is the sorted result of applying Selection Sort to [5, 5, 5, 5]?", options: ["[5, 5, 5, 5]", "Error — duplicate elements not handled", "[0, 0, 0, 5]", "Undefined behavior"], answerIndex: 0, hint: "The algorithm works correctly with duplicate elements." },
          { question: "Selection Sort is most preferred over Bubble Sort in which scenario?", options: ["When the array is nearly sorted", "When memory writes are expensive and minimizing swaps is critical", "When the array has many duplicate elements", "When stability of sort is required"], answerIndex: 1, hint: "Its advantage is its minimal number of swaps." }
        ],
        references: [
          "Weiss M.A., Data Structures and Algorithm Analysis in C, 2nd Edition, Pearson",
          "Sedgewick R., Algorithms in C Parts 1-5, Addison-Wesley",
    ...
    },
    {
      ...
      content: {
        ...
        pretest: [
          { question: "In Insertion Sort, what is meant by the key element?", options: ["The minimum element in the array", "The element currently being inserted into its correct position in the sorted portion", "The element at the middle index", "The first element of the array"], answerIndex: 1, hint: "It's the element you're trying to place into the already-sorted left part." },
          { question: "What is the best-case time complexity of Insertion Sort and when does it occur?", options: ["O(n²) — when the array is reverse sorted", "O(n) — when the array is already sorted", "O(log n) — when the array has no duplicates", "O(1) — when all elements are equal"], answerIndex: 1, hint: "In this case, the inner loop only performs a single comparison and then exits." },
          { question: "In Insertion Sort what operation makes room for the key element to be inserted?", options: ["Swapping adjacent elements like Bubble Sort", "Selecting the minimum and swapping like Selection Sort", "Shifting elements one position to the right", "Dividing the array into two halves"], answerIndex: 2, hint: "This is more efficient than swapping for making space." },
          { question: "After i passes of Insertion Sort, which portion of the array is guaranteed to be sorted?", options: ["The last i elements", "The first i+1 elements", "The middle i elements", "No guarantee until all passes are complete"], answerIndex: 1, hint: "The algorithm builds the sorted list from left to right." },
          { question: "Which real-world scenario best illustrates Insertion Sort?", options: ["Searching for a name in a phonebook", "Sorting a hand of playing cards by inserting each new card into its correct position", "Dividing tasks equally among workers", "Finding the tallest person in a group"], answerIndex: 1, hint: "This is the classic analogy used to explain Insertion Sort." }
        ],
        procedure: [
        ...
        posttest: [
          { question: "For array [1, 2, 3, 4, 5] (already sorted), how many comparisons does Insertion Sort make?", options: ["0", "n-1 = 4", "n(n-1)/2 = 10", "n² = 25"], answerIndex: 1, hint: "The inner loop's condition `A[j] > key` will be false for each key, so it only performs the comparison and then moves on." },
          { question: "What is the array state after i=2 (third pass) of Insertion Sort on [5, 4, 3, 2, 1]?", options: ["[3, 4, 5, 2, 1]", "[1, 2, 3, 4, 5]", "[3, 4, 5, 2, 1]", "[3, 4, 5, 2, 1]"], answerIndex: 0, hint: "Let's trace: Pass i=1 (key=4): [4, 5, 3, 2, 1]. Pass i=2 (key=3): [3, 4, 5, 2, 1]." },
          { question: "Why is Insertion Sort preferred over Bubble Sort and Selection Sort for nearly sorted arrays?", options: ["It has lower worst-case complexity", "It is adaptive — its inner loop exits early when the correct position is found quickly requiring very few comparisons and shifts for nearly sorted input", "It uses less memory", "It performs fewer total passes"], answerIndex: 1, hint: "This is its key advantage over the other basic O(n²) sorts." },
          { question: "Insertion Sort is classified as a stable sorting algorithm. Which of the following confirms this?", options: ["It always runs in O(n) time", "Equal elements are never swapped past each other — the while loop condition uses strictly greater than so equal elements are not shifted", "It uses a single temporary variable", "The sorted portion always grows from left to right"], answerIndex: 1, hint: "The condition `A[j] > key` is `>` not `>=`. This prevents shifting elements that are equal to the key." },
          { question: "Which modern sorting algorithm uses Insertion Sort as a building block for small subarrays?", options: ["Quick Sort", "Heap Sort", "TimSort (used in Python and Java)", "Radix Sort"], answerIndex: 2, hint: "Many hybrid sorts switch to Insertion Sort for small partitions because it has low overhead." }
        ],
        references: [
          "Weiss M.A., Data Structures and Algorithm Analysis in C, 2nd Edition, Pearson",
